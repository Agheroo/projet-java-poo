        ...
        try {
            if (animated) {
                for (int j = 0; j < spriteCntMax; j++) {
                    image[j] = ImageIO.read(new FileInputStream("/home/romu/IdeaProjects/projet-java-poo/res/tiles/animated/" + name + "/" + name + (j + 1) + ".png"));
                }
            } else {
                image[0] = ImageIO.read(new FileInputStream("/home/romu/IdeaProjects/projet-java-poo/res/tiles/static/" + name + "/" + name + (i + 1) + ".png"));
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
